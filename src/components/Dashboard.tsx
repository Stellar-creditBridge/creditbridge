...
export default function Dashboard({ invoices, investments = [], activities, onSubmitInvoice, onRepayInvoice, onUpdateRisk, setView, wallet, theme }: DashboardProps) {
  ...
  // Live Stellar Network Telemetry via Horizon Monitor Hook
  const { 
    networkStatus, 
    isLoading: isNetworkLoading, 
    isRefreshing: isNetworkRefreshing, 
    error: networkError, 
    refresh: refreshNetwork 
  } = useStellarNetwork({ pollIntervalMs: 12000 });
          ...
          <div className="bg-zinc-50 border border-black/10 p-4 rounded-none text-left space-y-3" id="stellar-network-monitor">
            <div className="flex items-center justify-between border-b border-black/5 pb-2">
              <span className="text-[10px] font-mono font-bold text-black uppercase tracking-wider flex items-center gap-1.5">
                {isNetworkLoading && !networkStatus ? (
                ...
                <span>Stellar {networkStatus?.network === 'public' ? 'Public Network' : 'Testnet'}</span>
                {networkStatus?.isStale && (
                  <span className="ml-1 px-1 py-0.2 text-[7px] font-mono uppercase bg-amber-100 text-amber-800 border border-amber-300">
                    Stale Cache
                  </span>
                )}
              </span>
              <div className="flex items-center gap-2">
                <span className="text-[8px] font-mono font-bold text-zinc-400 uppercase">
                  {networkStatus?.horizonVersion ? `Horizon ${networkStatus.horizonVersion.split('-')[0]}` : 'Horizon API'}
                </span>
                <button
                  type="button"
                  onClick={() => refreshNetwork(true)}
                  disabled={isNetworkRefreshing}
                  title="Force refresh live network telemetry from Horizon"
                  className="p-1 text-zinc-400 hover:text-black transition-colors cursor-pointer disabled:opacity-50"
                >
                  <RefreshCw className={`w-3 h-3 ${isNetworkRefreshing ? 'animate-spin text-black' : ''}`} />
                </button>
              </div>
            </div>

            {/* Content States: Loading, Unavailable, or Live Metrics */}
            {isNetworkLoading && !networkStatus ? (
              /* Loading Skeleton (No Fake Numbers) */
              <div className="space-y-2 py-2 font-mono" id="stellar-monitor-loading">
                <div className="flex items-center gap-2 text-zinc-400 text-[9px]">
                  <Database className="w-3 h-3 animate-pulse text-zinc-500" />
                  <span>Connecting to Stellar Horizon node...</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-zinc-100 animate-pulse h-12 border border-black/5"></div>
                  <div className="bg-zinc-100 animate-pulse h-12 border border-black/5"></div>
                  <div className="bg-zinc-100 animate-pulse h-12 border border-black/5"></div>
                  <div className="bg-zinc-100 animate-pulse h-12 border border-black/5"></div>
                </div>
              </div>
            ) : networkStatus?.status === 'unavailable' || (!networkStatus?.latestLedger) ? (
              /* Degraded / Unavailable State (No Fake Numbers) */
              <div className="p-3 bg-rose-50 border border-rose-200 text-left space-y-2" id="stellar-monitor-unavailable">
                <div className="flex items-center gap-1.5 text-rose-800 font-mono text-[9px] font-bold uppercase tracking-wider">
                  <WifiOff className="w-3.5 h-3.5 shrink-0" />
                  <span>Horizon Telemetry Unavailable</span>
                </div>
                <p className="text-[9px] text-rose-700 font-mono leading-relaxed">
                  {networkStatus?.error || networkError || 'Unable to establish connection to Stellar Horizon gateway.'}
                </p>
                <div className="pt-1 flex items-center justify-between">
                  <span className="text-[8px] font-mono text-rose-600 truncate max-w-[180px]">
                    {networkStatus?.horizonUrl || 'https://horizon-testnet.stellar.org'}
                  </span>
                  <button
                    type="button"
                    onClick={() => refreshNetwork(true)}
                    className="px-2 py-1 bg-rose-600 hover:bg-rose-700 text-white font-mono text-[8px] font-bold uppercase tracking-wider cursor-pointer"
                  >
                    Retry Handshake
                  </button>
                </div>
              ...
              /* Genuine Live Stellar Network Metrics */
              <div className="space-y-2" id="stellar-monitor-live-data">
                {networkStatus.isStale && (
                  <div className="px-2 py-1 bg-amber-50 border border-amber-200 text-amber-800 text-[8px] font-mono flex items-center gap-1">
                    <AlertTriangle className="w-2.5 h-2.5 shrink-0" />
                    <span>Horizon temporarily unreachable; showing last-verified ledger snapshot.</span>
                  </div>
                )}
                
                <div className="grid grid-cols-2 gap-3 font-mono text-[10px]">
                  <div className="bg-white border border-black/5 p-2 flex flex-col justify-between">
                    <span className="text-zinc-400 uppercase text-[8px] font-bold">Ledger Sequence</span>
                    <div className="flex items-center justify-between mt-1">
                      <span className="font-bold text-black truncate" title={`Ledger #${networkStatus.latestLedger.sequence}`}>
                        #{networkStatus.latestLedger.sequence.toLocaleString()}
                      </span>
                      <a
                        href={getStellarExplorerUrl('ledger', networkStatus.latestLedger.sequence, networkStatus.network)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-zinc-400 hover:text-black transition-colors"
                        title="View ledger on Stellar.Expert block explorer"
                      >
                  ...
                  <div className="bg-white border border-black/5 p-2 flex flex-col justify-between">
                    <span className="text-zinc-400 uppercase text-[8px] font-bold">Ledger Close</span>
                    <div className="flex items-baseline justify-between mt-1">
                      <span className="font-bold text-black">
                        {networkStatus.latestLedger.closeTimeSeconds.toFixed(1)}s
                      </span>
                      <span className="text-[8px] text-zinc-400">Target ~5s</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-black/5 p-2 flex flex-col justify-between">
                    ...
                    <div className="flex items-baseline justify-between mt-1">
                      <span className="font-bold text-emerald-600">
                        {networkStatus.latestLedger.baseFee} Stroops
                      </span>
                      {networkStatus.feeStats && (
                        <span className="text-[7.5px] text-zinc-400" title="Mode accepted transaction fee">
                          Mode {networkStatus.feeStats.modeAcceptedFee}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-black/5 p-2 flex flex-col justify-between">
                    <span className="text-zinc-400 uppercase text-[8px] font-bold">Protocol Core</span>
                    <div className="flex items-baseline justify-between mt-1">
                      <span className="font-bold text-black">
                        Protocol v{networkStatus.protocolVersion ?? 28}
                      </span>
                      <span className="text-[8px] text-zinc-400">
                        {networkStatus.latencyMs ? `${networkStatus.latencyMs}ms` : 'Active'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-1.5 border-t border-black/5 flex items-center justify-between text-[8px] font-mono text-zinc-400">
                  <span>
                    {networkStatus.latestLedger.txCount} txs {networkStatus.feeStats ? `• ${(networkStatus.feeStats.capacityUsage * 100).toFixed(0)}% load` : ''}
                  </span>
                  <span className="flex items-center gap-1">
                    {networkStatus.isCached && !networkStatus.isStale && (
                      <span className="px-1 py-0.2 bg-zinc-100 text-zinc-600 font-bold uppercase text-[7px]">Cached</span>
                    )}
                    <span>Synced {new Date(networkStatus.fetchedAt).toLocaleTimeString()}</span>
                  </span>
                </div>
              </div>
            )}
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center justify-between gap-2 bg-white border border-black/5 p-2 rounded-none">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <a
                      href={getStellarExplorerUrl('account', walletAddress, networkStatus?.network || 'testnet')}
                      target="_blank"
                      rel="noopener noreferrer"
                      title="View account on Stellar.Expert block explorer"
                      className="p-1 hover:bg-black/5 text-zinc-500 hover:text-black transition-colors cursor-pointer"
                    >
